...
public class HeadCommand extends SubCommand {
    public HeadCommand(PosableArmorStands plugin, CommandManager commandManager) {
    ...
    public String description(Player player) {
        return ConfigurationManager.getInstance().getLanguageValue(player, LanguageLookup.Commands_Head_Description);
    }

    @Override
    public String usage(Player player) {
        return ConfigurationManager.getInstance().getLanguageValue(player, LanguageLookup.Commands_Head_Usage);
    }
    ...
    public void Execute(Player player, String[] args) {
        ArmorStandManager.getInstance().selectMotionPart(player, Commands.Head, Double.parseDouble(args[1]));
    }
}
