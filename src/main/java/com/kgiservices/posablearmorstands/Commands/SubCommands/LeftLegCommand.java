...
public class LeftLegCommand extends SubCommand {
    public LeftLegCommand(PosableArmorStands plugin, CommandManager commandManager) {
        super(plugin, commandManager);
    }

    @Override
    public Commands subCommandName() {
        return Commands.LeftLeg;
    }
    ...
    public String description(Player player) {
        return ConfigurationManager.getInstance().getLanguageValue(player, LanguageLookup.Commands_LeftLeg_Description);
    }

    @Override
    public String usage(Player player) {
        return ConfigurationManager.getInstance().getLanguageValue(player, LanguageLookup.Commands_LeftLeg_Usage);
    }
    ...
    public void Execute(Player player, String[] args) {
        ArmorStandManager.getInstance().selectMotionPart(player, Commands.LeftLeg, Double.parseDouble(args[1]));
    }
}
