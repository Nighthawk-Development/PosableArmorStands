...
public class BodyCommand extends SubCommand {
    ...
    public String description(Player player) {
        return ConfigurationManager.getInstance().getLanguageValue(player, LanguageLookup.Commands_Body_Description);
    }

    @Override
    public String usage(Player player) {
        return ConfigurationManager.getInstance().getLanguageValue(player, LanguageLookup.Commands_Body_Usage);
    }
    ...
    public void Execute(Player player, String[] args) {
        ArmorStandManager.getInstance().selectRotationPart(player, Commands.Body, Double.parseDouble(args[1]));
    }
}
