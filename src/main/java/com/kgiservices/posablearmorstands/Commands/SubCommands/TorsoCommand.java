...
public class TorsoCommand extends SubCommand {
    public TorsoCommand(PosableArmorStands plugin, CommandManager commandManager) {
        super(plugin, commandManager);
    }

    @Override
    public Commands subCommandName() {
        return Commands.Torso;
    }
    ...
    public String description(Player player) {
        return ConfigurationManager.getInstance().getLanguageValue(player, LanguageLookup.Commands_Torso_Description);
    }

    @Override
    public String usage(Player player) {
        return ConfigurationManager.getInstance().getLanguageValue(player, LanguageLookup.Commands_Torso_Usage);
    }
    ...
    public void Execute(Player player, String[] args) {
        ArmorStandManager.getInstance().selectMotionPart(player, Commands.Torso, Double.parseDouble(args[1]));
    }
}
